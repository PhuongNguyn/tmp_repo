...
const { baseModelName } = require("../models/taxonomy.model");
const slugHandler = require("../helpers/slugHandler");
const { dashLogger } = require("../logger");
exports.create = async (req, res) => {
  try {
      const { tax_name, tax_description, tax_type, tax_slug, tax_parent } =
        req.body;
      if (!tax_name || !tax_name.length) {
        dashLogger.error(
          `Error : Name is required, Request : ${req.originalUrl}`
        );
...
exports.read = async (req, res) => {
  ...
  try {
    if (slug) {
      await Taxonomy.findOne({ tax_slug: slug })
        .populate("tax_parent")
        .select(
          "id tax_name tax_description tax_type tax_slug tax_parent ancestors.slug ancestors.name ancestors._id"
        )
...
exports.descendants = async (req, res) => {
  try {
    ...
    if (tax_id) {
      ...
      dashLogger.error(
        `Error : Tax_id not found ,Request : ${req.originalUrl}`
      );
      return res.status(400).json({
...
exports.updateTaxonomy = async (req, res) => {
  ...
  try {
    if (id) {
        ...
        if (!id || !id.length) {
          dashLogger.error(
            `Error : Id is required,Request : ${req.originalUrl}`
          );
          return res.status(400).json({
        ...
        var slug =
          tax_slug == ""
            ? slugHandler.slugify(tax_name).toLowerCase()
            : tax_slug;
        if (new_parent_id != null) {
          //Thay đổi tổ tiên của một danh mục
          Taxonomy.findByIdAndUpdate(id, {
            $set: { tax_parent: new_parent_id },
          }).exec(async (err, data) => {
            if (err) {
              dashLogger.error(`Error : ${err},Request : ${req.originalUrl}`);
              return res.status(400).json({
        ...
        Taxonomy.findByIdAndUpdate(
          ...
          const updateName = await Taxonomy.updateMany(
            { "ancestors._id": id },
            {
              $set: {
  ...
  try {
    if (id) {
      Taxonomy.findByIdAndRemove(id).exec(async (err, data) => {
        ...
        var result = await Taxonomy.deleteMany({
