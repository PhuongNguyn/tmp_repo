...
async function createRoleAction(req, res) {
    if (req.actions.includes('createRoleAction')) {
        try {
			...
			dashLogger.error(`controller: roleActionController; Request: ${req.originalUrl}; Error: ${error}`);
...
async function createRoleActions(req, res) {
    if (req.actions.includes('createRoleActions')) {
        try {
            if(!req.body.role){
               return res.status(404).json(new ResponseModel(404, "Role was not found", null));
            }
            
            if(!req.body.actions){
              return res.status(404).json(new ResponseModel(404, "action was not found", null));
            }  
