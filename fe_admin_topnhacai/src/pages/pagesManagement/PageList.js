...
function PageList() {
  ...
  const handlePermission = async () => {
    let userPermission = permission.find((item) => item.name === "page");
  ...
  const onDelete = (record) => {
    deletePage(record._id)
      .then((res) => {
        notification["success"]({
          message: "Notification",
          description: "Delete page successfully!",
        });
        getPages();
      })
      .catch((error) => {
              ...
              <Col lg={12}>
                <Table dataSource={pageList} scroll={{ x: "max-content" }}>
                  ...
                  <Column
                    ...
                    key="page_title"
                    render={(item) => <>{formatString(item)}</>}
                  />
                  <Column
                    title="Đường dẫn tĩnh"
                    dataIndex="page_category_slug"
                    key="page_category_slug"
                  />
                  <Column
                    title="Mô tả"
                    ...
                    render={(val, record) => (
                      <Space size="middle">
                        {isPermission["edit"] && (
                          <>
                            <Link
                              to={{ pathname: "/pages-management/" + val._id }}
                            >
                              <i className="ri-eye-line action-icon"></i>
                            </Link>
                            <Link
                              to={{
                                pathname: "/pages-management/edit/" + val._id,
                              }}
                            >
