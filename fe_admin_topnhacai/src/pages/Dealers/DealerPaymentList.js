...
function DealerPaymentList() {
  ...
  const handlePermission = async () => {
    let userPermission = permission.find(
      (item) => item.name === "dealerPayment"
    );
  ...
  const onDelete = (record) => {
    deleteDealerPayment(record._id)
      .then((res) => {
        getDealers();
      })
      .catch((error) => {
  ...
  const onSearch = () => {
    if (!searchText) {
      ...
      searchDealerPayment({ q: searchText })
        .then((res) => {
          const formatRes = res.map((dealer) => ({
            ...dealer,
            key: dealer._id,
            ...
            {isPermission["add"] && (
              <Col lg="7">
                <div className="text-right">
                  <Button onClick={() => openPopupEditDealerPayment()}>
                    Thêm mới
                  </Button>
                </div>
              </Col>
          ...
          <Row>
            <Col lg={12}>
              ...
              >
                ...
                />
                <Column title="Tên" dataIndex="dealer_name" key="dealer_name" />
                <Column
                  title="Nạp tối thiểu"
                  dataIndex="dp_deposit_min"
                  key="dp_deposit_min"
                />
                <Column
                  title="Nạp tối đa"
                  dataIndex="dp_deposit_max"
                  key="dp_deposit_max"
                />
                <Column
                  title="Nạp tối đa/ngày"
                  dataIndex="dp_deposit_max_day"
                  key="dp_deposit_max_day"
                />
                <Column
                  title="Thời gian xử lý"
                  dataIndex="dp_deposit_processing_time"
                  key="dp_deposit_processing_time"
                />
                <Column
                  title="Cổng nạp"
                  dataIndex="dp_deposit_payments"
                  key="dp_deposit_payments"
                />

                <Column
                  title="Ngân hàng"
                  dataIndex="dp_deposit_banks"
                  key="dp_deposit_banks"
                  render={(items) => {
                    let array = Array.isArray(items)
                      ? JSON.parse(items)
                ...
                <Column
                  title="Rút tối thiểu"
                  dataIndex="dp_withdrawal_min"
                  key="dp_withdrawal_min"
                />
                <Column
                  title="Rút tối đa"
                  dataIndex="dp_withdrawal_max"
                  key="dp_withdrawal_max"
                />
                <Column
                  title="Rút tối đa/ngày"
                  dataIndex="dp_withdrawal_prc_time"
                  key="dp_withdrawal_prc_time"
                />
                <Column
                  title="Thời gian xử lý"
                  dataIndex="dp_withdrawal_payments"
                  key="dp_withdrawal_payments"
                />
                <Column
                  title="Ngân hàng hỗ trợ rút"
                  dataIndex="dp_withdrawal_banks"
                  key="dp_withdrawal_banks"
                  render={(items) => {
                    let array = Array.isArray(items)
                      ? JSON.parse(items)
                      : items && typeof items === "string"
                  ...
                  render={(val, record) => (
                    <Space size="middle">
                      {isPermission["edit"] && (
                        <>
                          <Link
                            to={{ pathname: "/dealers-payment/" + val._id }}
                          >
                          ...
                          <i
                            ...
                            onClick={() =>
                              openPopupEditDealerPayment(val, "edit")
                            }
                          ></i>
                        </>
                      )}
