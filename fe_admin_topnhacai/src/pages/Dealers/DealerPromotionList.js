...
import {
  getAllDealerPromotion,
  deleteDealerPromotion,
  searchDealerPromotion,
} from "../../helpers/helper";
...
function DealerPromotionList() {
  ...
  const handlePermission = async () => {
    let userPermission = permission.find(
      (item) => item.name === "dealerPromotion"
    );
  ...
  const onDelete = (record) => {
    deleteDealerPromotion(record._id)
      .then((res) => {
        getDealers();
      })
      .catch((error) => {
  ...
  const onSearch = () => {
    if (!searchText) {
      ...
      searchDealerPromotion({ q: searchText })
        .then((res) => {
          const formatRes = res.map((dealer) => ({
            ...dealer,
            key: dealer._id,
          ...
          <Row>
            <Col lg={12}>
              <Table rowKey="_id" dataSource={dealerList}>
                ...
                />
                <Column title="Tên" dataIndex="dp_name" key="dp_name" />
                <Column
                  title="Chi tiết"
                  dataIndex="dp_detail"
                  key="dp_detail"
                  ...
                  render={(val, record) => (
                    <Space size="middle">
                      {isPermission["edit"] && (
                        <>
                          <Link
                            to={{ pathname: "/dealers-promotion/" + val._id }}
                          >
                            <i className="ri-eye-line action-icon"></i>
                          </Link>
                          <Link
                            to={{
                              pathname: "/dealer-promotion/edit/" + val._id,
                            }}
                          >
