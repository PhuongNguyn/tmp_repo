...
function DealerServiceList() {
  ...
  const addDealer = () => {
    createDealerService(formVal)
      .then((res) => {
        notification["success"]({
          message: "Notification",
          description: "Create dealer service successfully!",
        });
        getDealers();
      })
      .catch((error) => {
  ...
  const onDelete = (record) => {
    deleteDealerService(record._id)
      .then((res) => {
        getDealers();
      })
      .catch((error) => {
  ...
  const onSearch = () => {
    if (!searchText) {
      ...
      searchDealerService({ q: searchText })
        .then((res) => {
          const formatRes = res.map((dealer) => ({
            ...dealer,
            key: dealer._id,
          ...
          <Row>
            <Col lg={12}>
              ...
              >
                ...
                <Column
                  title="Hotline"
                  dataIndex="ds_hotline"
                  key="ds_hotline"
                />
                <Column
                  title="Telegram"
                  dataIndex="ds_telegram"
                  key="ds_telegram"
                />
                <Column
                  ...
                  render={(val, record) => (
                    <Space size="middle">
                      {isPermission["edit"] && (
                        <>
                          <Link
                            to={{ pathname: "/dealers-service/" + val._id }}
                          >
                            <i className="ri-eye-line action-icon"></i>
                          </Link>
                          <Link
                            to={{ pathname: "/dealer-service/edit/" + val._id }}
                          >
