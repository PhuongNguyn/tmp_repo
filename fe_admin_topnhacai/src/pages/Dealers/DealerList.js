...
function DealerList() {
  ...
  const handlePermission = async () => {
    let userPermission = permission.find((item) => item.name === "dealer");
  ...
  const handleOk = (data) => {
    ...
    // onDelete(data);
    if (deleteItem._id) {
      deleteDealer(deleteItem._id)
        .then((res) => {
          notification["success"]({
            message: "Notification",
            description: "Delete dealer sucessfully!",
          });
          getDealers();
        })
        .catch((error) => {
            ...
            <Col>
              <Table dataSource={dealerList} scroll={{ x: "max-content" }}>
                ...
                <Column
                  ...
                  width={100}
                  render={(image) => (
                    <img src={image} alt="pro_image" style={{ width: "50%" }} />
                  )}
                />
                <Column
                  title="Thứ hạng"
                ...
                <Column
                  title="Khuyến mãi"
                  dataIndex="dealer_promotion"
                  key="dealer_promotion"
                  render={(val) => {
                    return (
                      <div>
                        {val && val?.length ? (
                ...
                <Column
                  ...
                  render={(val, record) => (
                    <Space size="middle">
                      {isPermission["edit"] && (
                        <>
                          <Link to={{ pathname: "/dealers/" + val._id }}>
                            <i className="ri-eye-line action-icon"></i>
                          </Link>
                          <Link to={{ pathname: "/dealers/edit/" + val._id }}>
