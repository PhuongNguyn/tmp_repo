...
const UserProfile = () => {
  ...
  const URLBUNNY = "https://cdn.topnhacai.today/"
  ...
  const onRemove = async () =>{
    ...
    try {
      await axios.patch(config.api.API_URL + '/api/user/editAvatar', {id: idx, image: ''})
  ...
  const uploadImage = async(options) => {
    ...
    if(result.Message == "File uploaded."){
      try {
          const result = await axios.patch(config.api.API_URL + '/api/user/editAvatar', {id: idx, image: file.name})
          const obj = JSON.parse(sessionStorage.getItem("authUser"));
  ...
  const handleUpdateProfile = async (e) =>{
    ...
    try {
      refLoading.current.continuousStart()
        await axios.patch(config.api.API_URL + '/api/user/editProfile', formData)
